...
def check_shot_validity(moments_window, peak_z):
    """Check two things: ball passes near basket AND had a real arc (not just a bounce)"""
    # Check 1: did the ball pass near either basket?
    near_basket = False
    ...
    # Check 2: was there a real arc? Peak must be meaningfully above the window's floor
    min_z_in_window = min(m['z'] for m in moments_window)
    has_arc = (peak_z - min_z_in_window) >= 2.0  # ball dropped at least 2 feet from peak

    return near_basket and has_arc
...
for i in range(1, len(all_moments) - 1):
    ...
    if not in_shot:
        ...
        if curr['z'] < DROP_THRESHOLD:
            ...
            # Hook shots: check ball passed near basket AND had a real arc (not a bounce)
            if current_shot_peak['z'] > SHOT_THRESHOLD or check_shot_validity(window_after, current_shot_peak['z']):
...
shots_detected = deduplicate_shots(shots_detected, min_gap=4.5)
